...
st.markdown(
    '<div class="subtitle">Main method: RBJ Parametric EQ. Includes smart presets, WAV/MP3 upload, before/after audio, FFT analysis, clipping protection, and filter-response visualization.</div>',
    unsafe_allow_html=True,
)
...
if page == "Home":
    ...
    st.markdown(
        """
        This project builds an interactive digital audio equalizer. It supports **WAV and MP3 upload**, and the main method is **RBJ Parametric EQ** because it is fast, low-latency, and suitable for real-time slider control.
    ...
    if uploaded is not None:
        ...
        st.session_state.processed_metrics = None
        st.session_state.last_settings = None
        st.success(f"Loaded {uploaded.name} successfully. WAV and MP3 files are supported.")
    ...
    if st.session_state.processed_audio is not None:
        ...
        show_clipping_status(metrics)
        show_audio_before_after()
        export_format = st.radio("Choose download format", ["WAV", "MP3"], horizontal=True)
        ...
        if export_format == "WAV":
            st.download_button(
                ...
                file_name=f"equalized_{safe_method_name}.wav",
                mime="audio/wav",
            )
        else:
            try:
                st.download_button(
                    "⬇️ Download processed MP3",
                    data=audio_to_mp3_bytes(y, fs),
                    file_name=f"equalized_{safe_method_name}.mp3",
                    mime="audio/mpeg",
                )
            except Exception as exc:
                st.error(f"MP3 export is not available in this environment: {exc}")
                st.info("You can still download the processed audio as WAV.")
